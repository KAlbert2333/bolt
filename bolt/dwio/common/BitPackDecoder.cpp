...
namespace {
  ...
  void gather8Sparse_sve(
    ...
    svuint32_t indices = svmla_u32_z(pg8, svdup_u32(static_cast<uint32_t>(bitOffset)), row_vec, svdup_n_u32(width));
    svuint32_t byte_indices = svlsr_u32_z(pg8, indices, svdup_n_u32(static_cast<uint32_t>(3)));

    svuint32_t data = svld1_gather_u32offset_u32(pg8, reinterpret_cast<const uint32_t*>(bits), byte_indices);

    if constexpr (width % 8 != 0) {
      constexpr static uint32_t kMultipliers_arr[8] = {256,128,64,32,16,8,4,2};
      svuint32_t kMultipliers_vec = svld1_u32(pg8, kMultipliers_arr);
      svuint32_t bit_in_byte = svand_u32_z(pg8, indices, svdup_n_u32(static_cast<uint32_t>(7)));
    ...
    for (; i + 8 <= numRows; i += 8) {
      ...
      if (width <= 16 && endRow - row == 7) {
        ...
        if (width <= 8) {
          ...
          svst1_u32(pg8, eightInts, svld1ub_u32(svwhilelt_b32(0, 8), (uint8_t*)&eightBytes));
        } else {
          // Use pdep to shift 2 words of bit packed data with width
          // 9-16. For widts <= 14 four bit packed fields can always be
          // loaded with a single uint64_t load. For 15 and 16 bits this
  ...
  #define WIDTH_CASE_SVE(width)                                                      \
    case width:                                                                      \
      i = decode1To24_sve<width>(bits, bitOffset, rows.data(), numSafeRows, result); \
      break;
